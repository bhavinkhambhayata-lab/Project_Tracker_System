        ...
        public ActionResult ExportReport(String SearchType = " ",int year = 0)
        {
            try
            {
                ...
                using (ExcelPackage pck = new ExcelPackage())
                {
                    ...
                    ws.Cells[headerRow + 1, col].Value = "";
                ...
                "Inco Terms","Payment Method","Payment terms","Remarks"
                    ...
                    // Dynamic Other Charges Subheaders
                    for (int i = 0; i < maxOtherChargesCount; i++)
                    {
                        ws.Cells[headerRow + 2, col].Value = "Vendor";
                        ws.Cells[headerRow + 2, col + 1].Value = "Invoice No";
                    ...
                    List<int> amountCols = new List<int>()
            {
                12,13,14,15,16,17,25,33,37,38,42,46,50,54,58
                    ...
                    for (int i = 0; i < maxOtherChargesCount; i++)
                    {
                        amountCols.Add(otherChargeStartCol + 3);
                        otherChargeStartCol += 4;
                    }

                    int totalExpCol = otherChargeStartCol;
                    int sqmCol = totalExpCol + 1;
                    int fclCol = totalExpCol + 2;

                    amountCols.Add(totalExpCol);
                    amountCols.Add(sqmCol);
                    amountCols.Add(fclCol);
                    ...
                    var chargeBlocks = new List<(int StartCol, int EndCol)>
            {
                ...
                (56,59)
                    ...
                    for (int i = 0; i < maxOtherChargesCount; i++)
                    {
                        chargeBlocks.Add((dynamicOtherStart, dynamicOtherStart + 3));
                        dynamicOtherStart += 4;
                    }

                    chargeBlocks.Add((dynamicOtherStart, dynamicOtherStart));
                    chargeBlocks.Add((dynamicOtherStart + 1, dynamicOtherStart + 1));
                    chargeBlocks.Add((dynamicOtherStart + 2, dynamicOtherStart + 2));
                    chargeBlocks.Add((dynamicOtherStart + 3, dynamicOtherStart + 6));
