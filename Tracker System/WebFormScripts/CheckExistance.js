...
function CheckExistanceForTenant(Value, ColumnName, TableName, WhereCondition, Message, IsMasterDB, IsEnterpriseLevel,TenantId) {
    var flag = false;
    $.ajax({
        cache: false,
        type: "POST",
        ...
        success: function (data) {
            if (data.Val == "1") {
                ...
                var msg = "";
                if (Message == "")
                    msg = data.Msg.replace("@param1", Value);
                else
                    msg = Message;
