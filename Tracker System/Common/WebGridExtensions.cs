...
public static class WebGridExtensions
{
        ...
        )
    {
        ...
        if (ModeEnabled(mode, WebGridPagerModes.FirstLast))
        {
            ...
            {
                InnerHtml = GridLink(webGrid, webGrid.GetPageUrl(0), firstText, 1)
            };
        ...
        if (ModeEnabled(mode, WebGridPagerModes.NextPrevious))
        {
            ...
            var part = new TagBuilder("li")
            {
                InnerHtml = GridLink(webGrid, webGrid.GetPageUrl(page), previousText, page)
            };
        ...
        if (ModeEnabled(mode, WebGridPagerModes.Numeric) && (totalPages > 1))
        {
            ...
            for (int i = first; i <= last; i++)
            {
                ...
                var part = new TagBuilder("li")
                {
                    InnerHtml = GridLink(webGrid, webGrid.GetPageUrl(i), pageText, i + 1)
                };
        ...
        if (ModeEnabled(mode, WebGridPagerModes.NextPrevious))
        {
            ...
            var part = new TagBuilder("li")
            {
                InnerHtml = GridLink(webGrid, webGrid.GetPageUrl(page), nextText, page + 1)
            };
        ...
        if (ModeEnabled(mode, WebGridPagerModes.FirstLast))
        {
            ...
            var part = new TagBuilder("li")
            {
                InnerHtml = GridLink(webGrid, webGrid.GetPageUrl(lastPage), lastText, lastPage+1)
            };
    ...
    private static String GridLink(WebGrid webGrid, string url, string text, int Pages)
    {
        TagBuilder builder = new TagBuilder("a");
        builder.SetInnerText(text);
        builder.MergeAttribute("href", "#");
        builder.MergeAttribute("onclick", "GetWebGridItemFilterData(" + Pages + ")");
